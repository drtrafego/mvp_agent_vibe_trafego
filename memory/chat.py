...
async def _fetch_from_db(phone: str) -> list[dict]:
    ...
    try:
        rows = await pool.fetch(
            """
            SELECT role, content FROM agente_trafego.chat_sessions
            WHERE phone = $1
...
async def save_messages(
    ...
    try:
        await pool.executemany(
            "INSERT INTO agente_trafego.chat_sessions (phone, role, content, message_type, media_id) VALUES ($1, $2, $3, $4, $5)",
            [
...
async def create_table_if_not_exists() -> None:
    ...
    pool = await _get_pool()
    try:
        await pool.fetchval("SELECT 1 FROM agente_trafego.chat_sessions LIMIT 1")
