...
export default async function DealDetailPage({
  ...
  const dealActivities = await db.select().from(activities).where(eq(activities.dealId, id)).orderBy(desc(activities.createdAt));
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          {contact && (
            <Link href={`/contacts/${contact.id}`} className="text-muted-foreground hover:text-primary text-sm">
              {contact.name}
            </Link>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card><CardContent className="pt-6">
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1"><DollarSign className="h-4 w-4" />Valor</div>
        ...
        <Card><CardContent className="pt-6">
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1"><DollarSign className="h-4 w-4" />Valor ponderado</div>
          <p className="text-xl font-bold">{formatCurrency(Math.round(deal.value * (deal.probability / 100)))}</p>
        </CardContent></Card>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {deal.notes && (
          <Card><CardHeader><CardTitle className="text-base">Notas</CardTitle></CardHeader>
            ...
            {dealActivities.length === 0 ? (
              ...
              <div className="space-y-3">
                {dealActivities.map((activity) => {
                  const config = ACTIVITY_TYPE_CONFIG[activity.type as keyof typeof ACTIVITY_TYPE_CONFIG];
                  return (
                    <div key={activity.id} className="flex gap-3 items-start">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">{formatRelativeDate(activity.createdAt as number | Date)}</span>
                        </div>
