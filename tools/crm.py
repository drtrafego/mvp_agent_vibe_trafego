...
async def get_contact(phone: str) -> dict:
    ...
    row = await pool.fetchrow(
        "SELECT * FROM agente_trafego.contacts WHERE phone = $1 LIMIT 1", phone
    )
    if row:
        return dict(row)
    ...
    result = await pool.fetchrow(
        """
        INSERT INTO agente_trafego.contacts (id, phone, name, stage, followup_count)
...
async def update_contact(phone: str, **kwargs) -> None:
    ...
    await pool.execute(
        f"UPDATE agente_trafego.contacts SET {sets}, updated_at = now() WHERE phone = $1",
        phone, *values,
    )
...
async def append_observation(phone: str, obs: str) -> None:
    ...
    pool = await _get_pool()
    row = await pool.fetchrow(
        "SELECT observacoes_sdr FROM agente_trafego.contacts WHERE phone = $1", phone
    )
    ...
    await pool.execute(
        "UPDATE agente_trafego.contacts SET observacoes_sdr = $2, updated_at = now() WHERE phone = $1",
        phone, "\n".join(lines),
    )
...
async def advance_stage(phone: str, new_stage: str) -> None:
    ...
    pool = await _get_pool()
    await pool.execute(
        "UPDATE agente_trafego.contacts SET stage = $2, updated_at = now() WHERE phone = $1",
        phone, new_stage,
    )
...
async def mark_bot_message(phone: str) -> None:
    ...
    pool = await _get_pool()
    await pool.execute(
        "UPDATE agente_trafego.contacts SET last_bot_msg_at = now(), updated_at = now() WHERE phone = $1",
        phone,
    )
